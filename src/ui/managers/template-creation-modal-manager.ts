...
export async function writeTemplateFile(app: App, folder: string, spec: TemplateSpec): Promise<string> {
    const safeFolder = folder.replace(/^\/+/, "");
    const fileName = spec.name.replace(/\.xopp$/i, "") + ".xopp";
    const vaultPath = normalizePath(safeFolder ? `${safeFolder}/${fileName}` : fileName);
    ...
    if (spec.gzip) {
        ...
        const arrayBuffer = gzipped.buffer.slice(gzipped.byteOffset, gzipped.byteOffset + gzipped.byteLength);
...
export async function createTemplate(plugin: XoppPlugin, spec: TemplateSpec): Promise<string> {
    if (!spec.name) {
        new Notice("Please enter a template name");
        throw new Error("Missing template name");
    }

    const folder = plugin.settings.templatesFolder?.trim() || "XournalTemplates";
