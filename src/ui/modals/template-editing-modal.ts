...
import { TemplateSpec } from "./template-creation-modal";
...
export default class TemplateEditingModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(contentEl).setName("Choose a template").addDropdown(async (dropdown) => {
            ...
            dropdown.onChange(async (value) => {
                await parseTemplateFile(this.plugin, value, () => {
                    this.close();
                });
            });
        });
...
export class ParsedTemplateEditing extends Modal {
    ...
    onOpen() {
        ...
        new Setting(contentEl).setName("Orientation").addDropdown((dropdown) => {
            dropdown
        ...
        new Setting(contentEl).setName("Background Style").addDropdown((dropdown) => {
            dropdown
                ...
                .onChange((value) => {
                    spec.backgroundStyle = value as TemplateSpec["backgroundStyle"];
                });
        });
